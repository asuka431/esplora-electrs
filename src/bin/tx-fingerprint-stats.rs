...
fn main() {
    ...
    use baricoin::blockdata::script::Script;
    use baricoin::consensus::encode::deserialize;
    use electrs::{
        chain::Transaction,
        config::Config,
        daemon::Daemon,
    ...
    let mut indexer = Indexer::open(Arc::clone(&store), FetchFrom::Baricoind, &config, &metrics);
